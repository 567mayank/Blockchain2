...
const Coin = ({ name, symbol, price, change, marketcap,onClick }) => {
  const imageUrl = `https://assets.coincap.io/assets/icons/${symbol.toLowerCase()}@2x.png`;
  const handleClick = () => {
    onClick({ name, symbol, price, change });
  };
  return (
    <div className='grid grid-cols-[1fr_1fr_1fr_1fr] mb-4 border-b-[2px] border-[solid] border-[black] pb-1'>
      <div className='flex flex-row gap-x-4' onClick={handleClick}>
